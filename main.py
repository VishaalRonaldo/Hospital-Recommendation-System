...
def get_hospitals_for_disease(disease_name, limit=8):
    disease_name = disease_name.strip().lower()
    hospital_data['Disease'] = hospital_data['Disease'].astype(str).str.strip().str.lower()

    matched = hospital_data[hospital_data['Disease'] == disease_name]

    # Limit to 5 to 8 hospitals max (you can adjust `limit`)
    return list(zip(matched['Hospital_Name'], matched['Location']))[:limit]
...
def home():
    if request.method == 'POST':
        ...
        if symptoms == "Symptoms":
            ...
            hospital_list = get_hospitals_for_disease(predicted_disease, limit=8)
            my_precautions = []
            ...
            return render_template('index.html', symptoms=corrected_symptoms, predicted_disease=predicted_disease, dis_des=dis_des,
                                   ...
                                   workout=workout,hospital_list=hospital_list)
